...
/**
 * @Module lootsheetnpct2k4e.Helpers.ChatHelper
 *
...
export class ChatHelper {
    ...
    static async tradeChatMessage(source, destination, movedItems, options = { chatOutPut: true, verbose: false }) {
        ...
        if (existingItems.id) {
            ...
            messageObject.flags.lootsheetnpct2k4e.lootId = interactionId;
    ...
    static _getItemsFromLootMessage(interactionId, options = { verbose: true }) {
        let lootMessage = game.messages.filter(m => m.data.flags?.lootsheetnpct2k4e?.lootId == interactionId).pop(),
